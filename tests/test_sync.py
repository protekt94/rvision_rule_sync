...
@pytest.fixture
def pair():
    return read(FIX / 'RV-D-971.package.ro'), read(FIX / 'VI-D-971.ro')


def test_real_pair_roundtrip(pair):
    for rule in pair:
...
def test_auto_merge_real_custom(pair):
    b, o = pair
    t = copy.deepcopy(b)
    t['filter'] = VRL('true\n')
    r = compare(b, o, t)
...
def test_metadata(pair):
    b, _ = pair
    o = dict(b, id='VI-D-971', rid='custom-rid')
...
def test_package_cli(tmp_path):
    ws = tmp_path / 'workspace'
    assert main(['bootstrap', '--workspace', str(ws), '--base', str(FIX / 'RV-D-971.package.ro'),
                 '--custom', str(FIX / 'VI-D-971.ro'), '--baseline-verified']) == 0
    assert main(['analyze', '--workspace', str(ws), str(FIX / 'SIEM_Content_CIMv2_22092026.roc')]) == 0
    result = next((ws / 'runs').iterdir())
    assert json.loads((result / 'results.json').read_text())[0]['status'] == 'NO_CHANGE'
    assert (result / 'BASE/RV-D-971.ro').exists()
    assert not list((result / 'generated').iterdir())
    with pytest.raises(SyncError):
        bootstrap(ws, FIX / 'RV-D-971.package.ro', FIX / 'VI-D-971.ro')


def test_runs_do_not_reuse_candidates(tmp_path, pair):
    b, o = pair
    ws = bootstrap(tmp_path / 'ws', FIX / 'RV-D-971.package.ro', FIX / 'VI-D-971.ro', True)
    t = dict(b, filter=VRL('true\n'))
    vendor = tmp_path / 'vendor.ro'
    vendor.write_text(dump(t))
    first, _ = analyze(ws, vendor)
    assert (first / 'generated/VI-D-971.ro').exists()
    ...
    assert read(ws / 'custom/VI-D-971.ro') == o
    (ws / 'base/RV-D-971.ro').write_text('id: RV-D-971')
    with pytest.raises(SyncError):
        analyze(ws, vendor)


def test_original_pair():
    b, o, t = [read(FIX / n) for n in ('RV-D-971.ro', 'VI-D-971.ro', 'RV-D-971.package.ro')]
...
def test_empty_vendor_orphan(tmp_path):
    ws = bootstrap(tmp_path / 'ws', FIX / 'RV-D-971.package.ro', FIX / 'VI-D-971.ro', True)
...
def test_direct_real_pair(pair):
    vendor, ours = pair
    r = compare_pair(ours, vendor)
...
def test_direct_statuses():
    ours = {'id': 'VI-D-1', 'rid': 'custom', 'filter': VRL('true\n')}
    vendor = dict(ours, id='RV-D-1', rid='vendor')
    assert compare_pair(ours, vendor)['status'] == 'NO_CHANGE'
    vendor['description'] = 'new'
    assert compare_pair(ours, vendor)['status'] == 'METADATA_ONLY'
...
def test_default_cli_repeated_and_new_custom(tmp_path, monkeypatch):
    ...
    Path('custom/another.ro').write_text('id: VI-D-2\n')
    assert main(['compare']) == 2
    second = next(iter(set(Path('reports').iterdir()) - first))
    results = json.loads((second / 'results.json').read_text())
    assert len(results) == 2
    assert next(r for r in results if r['id'] == 'VI-D-2')['status'] == 'ORPHANED'
...
def test_multiple_packages_no_cross_contamination(tmp_path):
    ...
    _, results = direct_compare(custom, [folder], tmp_path / 'reports')
...
def test_no_inputs(tmp_path):
    ...
    with pytest.raises(SyncError, match='No VI-'):
        direct_compare(tmp_path, [tmp_path], tmp_path / 'reports')
...
def test_corrupt_package_does_not_publish_report(tmp_path):
    ...
    with pytest.raises(SyncError):
        direct_compare(custom, [valid, corrupt], tmp_path / 'reports')
